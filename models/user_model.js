...
async function getUsers(page_no = 1) {
  ...
  const result = await db.query(
    `SELECT * FROM users WHERE id > 6 LIMIT ${offset},${config.listPerPage}`
  );
...
async function updateUser(id, title, contents) {
  const result = await db.query(
    "UPDATE users SET title=?, contents=? WHERE id=?",
    [title, contents, id]
  );
...
async function searchUsersByTitle(title) {
  const result = await db.query(
    `SELECT * FROM users WHERE (title LIKE '%${title}%') OR (contents LIKE '%${title}%')`
  );
